import { c as create_ssr_component, e as escape, v as validate_component, f as createEventDispatcher, a as subscribe, s as setContext, g as set_store_value, b as add_attribute, h as getContext, i as hasContext, o as onDestroy } from './index3-046add8a.js';
import { w as writable } from './index2-2089616d.js';
...
const Pane = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$unsubscribe_showFirstSplitter();
  return `${!gathering ? `
	${$veryFirstPaneKey !== key || $showFirstSplitter ? `
		
		<div class="${"splitpanes__splitter " + escape("", true)}"></div>` : ``}

	
	
	
	<div${add_attribute("class", `splitpanes__pane ${clazz || ""}`, 0)}${add_attribute("style", style, 0)}${add_attribute("this", element, 0)}>${slots.default ? slots.default({}) : ``}</div>` : ``}`;
});
const css = {
...
const CodeEditor = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  let is_owner = false;
  let user1;
  user.subscribe((value2) => {
    if (value2) {
      user1 = JSON.parse(value2);
  ...
  do {
    $$settled = true;
    $$rendered = `${!is_owner ? `<span>*You are not the owner of this project</span>` : ``}
${validate_component(Splitpanes, "Splitpanes").$$render(
      $$result,
      {
        style: "height: 90vh",
        class: "splitpanes"
      ...
      {
        default: () => {
          return `${validate_component(Pane, "Pane").$$render($$result, { minSize: 20, size: 60 }, {}, {
            default: () => {
              return `${validate_component(CodeMirror, "CodeMirror").$$render(
                $$result,
                {
                  lang: python(),
                  styles: {
	...
	${validate_component(Pane, "Pane").$$render($$result, { minSize: 20 }, {}, {
            default: () => {
              return `${`<p>Creating Container...</p>`}`;
            }
          })}`;
        }
      }
...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  fetch(url).then(async (response) => {
    if (response.ok) {
      let data1 = await response.json();
      data1["results"][0].code;
    }
  }).catch((error) => {
  });
  if ($$props.data === void 0 && $$bindings.data && data !== void 0)
    $$bindings.data(data);
  return `${user1 ? `<nav class="${"split-nav"}"><div class="${"nav-brand"}"><h5><a href="${"/"}">Home</a> &gt; <a href="${"/write-code-online"}">All Projects</a> &gt; ${escape(data.name)}</h5></div>
    <div class="${"collapsible"}"><input id="${"collapsible1"}" type="${"checkbox"}" name="${"collapsible1"}">
      ...
      <div class="${"collapsible-body"}"></div></div></nav>


${validate_component(CodeEditor, "CodeEditor").$$render($$result, { projectdata: data }, {}, {})}` : `${validate_component(LoginRequired, "LoginRequired").$$render($$result, {}, {}, {})}`}`;
});

export { Page as default };
//# sourceMappingURL=_page.svelte-d2bdd702.js.map
